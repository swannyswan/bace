...
SELECT gen_grid_thetas(theta => ARRAY[
    ...
    [-5, 5, 1], -- Parameter 1 values will be: [-5, -4, ..., 4, 5] (value of small trees — Quality X)
    [-5, 5, 1], -- Parameter 2 values will be: [-5, -4, ..., 4, 5] (value of large trees — Quality Y)
    [-5, 5, 1], -- Parameter 3 values will be: [-5, -4, ..., 4, 5] (value of grass — Quality Z)
    [-1.5, 1.5, 0.5], -- Parameter 4 values will be: [-1.5, -1, -0.5, 0, 0.5, 1, 1.5] (value of (X|Y)^Z)
--     [-1.5, 1.5, 0.5], -- Parameter 5 values will be: [-1.5, -1, -0.5, 0, 0.5, 1, 1.5]
    [1, 5, 2] -- Parameter 5 values will be: [1, 3, 5]
]);

/*
    STEP 2: CREATE Designs - Grid of possible discrete choice designs that can be asked
    design_structure - table with discrete choice values. 3 characteristics and an interaction term.
...
/* [small_trees, big_trees, grass, grass*trees] */
...
-- END;
BEGIN
    -- util_diff = u(treat) - u(base)    -- Note, PostgreSQL arrays are 1-based. E.g. IF x = ARRAY[10, 4, 7], then x[1] = 10, x[2] = 4, ...
    --     -- util_diff = u(treat) - u(base)
    util_diff := design[1] + design[2] * theta[1] + design[3] * theta[2] + design[4] * theta[3] + design[5] * theta[4];
    -- Likelihood of choosing the treated option
